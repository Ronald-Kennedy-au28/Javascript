...
// Retrieving saved tickets of previous sessions from LocalStorage
...
// "Open and Close of Task Container
...
function createTicket(ticketId,text,color){
    ...
    ticket.addEventListener("click",()=>{
        if(deleteColor){
            ticket.remove()  // removes the ticket from UI
            let index = ticketArr.findIndex((tick)=>{ // finding index to remove element from storage
                return tick.id == id;
            })
    ...
    // Tickets lock-Unlock function and TextArea Editability
...
// Displaying tickets according Color-click from the toolBox
for(let i=0;i<allPriorityColor.length;i++){
    ...
    // Double click of color from toolBox for all ticket display
